...
def readESOGUProblemInstances(file):
    """
    Verilen dosyadan rota problemi örneklerini okuyan fonksiyon.
    ...
    with open(file) as f:
        ...
        while target_line != '\n':
            ...
            if stl[1] == 'd':
                depot = Target(stl[0], idx, int(float(stl[2])), int(float(stl[3])), int(float(stl[6])),
                            int(float(stl[7])), int(float(stl[5])))
            elif stl[1] == 'f':
                new_target = ChargeStation(stl[0], idx, int(float(stl[2])), int(float(stl[3])), int(float(stl[6])),
                                            int(float(stl[7])), int(float(stl[5])))
                fuel_stations.append(new_target)
            elif stl[1] == 'c':
                new_target = Customer(stl[0], idx, int(float(stl[2])), int(float(stl[3])), int(float(stl[4])),
                                    int(float(stl[6])), int(float(stl[7])), int(float(stl[5])))
                customers.append(new_target)
